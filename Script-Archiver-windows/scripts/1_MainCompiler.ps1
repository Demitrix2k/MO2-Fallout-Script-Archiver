...
# 3. Create BSA archives with ESP plugins
...
function Get-RelativePathFromMod {
    ...
    # Ensure ModPath ends with a backslash for proper path trimming
    if (-not $absoluteModPath.EndsWith("\")) {
    ...
    for ($i = 0; $i -lt $Categories.Count; $i++) {
        ...
        if ($AutomaticMode) {
            ...
            try {
                ...
                # Execute BSArch using direct argument passing to avoid path issues
                if ($CompressArchives) {
                    $result = & $BSArchPath "pack" $compilationPath $archiveDestination "-$GameFormat" "-z" "-share" "-mt" 2>&1
                } else {
                    $result = & $BSArchPath "pack" $compilationPath $archiveDestination "-$GameFormat" "-share" "-mt" 2>&1
                }
...
function Create-SingleBSAArchive {
    ...
    if ($AutomaticMode) {
        ...
        try {
            ...
            # Use & with individual arguments instead of argument array to avoid path issues
            if ($CompressArchives) {
                $result = & $BSArchPath "pack" $sourcePath $archiveDestination "-$GameFormat" "-z" "-share" "-mt" 2>&1
            } else {
                $result = & $BSArchPath "pack" $sourcePath $archiveDestination "-$GameFormat" "-share" "-mt" 2>&1
            }
